...
def create_hdf5_for_subset(save_path, rep_path, ids, num_folds=2, fold=0, scanpy_package=rsc, resolution=1):
    ...
    train_cases = [case_id for case_id in case_numbers if (case_id%NUM_FOLDS != fold) & (case_id in ids)]
    valid_cases = [case_id for case_id in case_numbers if (case_id%NUM_FOLDS == fold) & (case_id in ids)]
